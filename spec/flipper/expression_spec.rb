...
RSpec.describe Flipper::Expression do
  ...
  describe "#in_words" do
    ...
    it "returns human-readable text for LessThan expression" do
      expression = described_class.build({"LessThan" => ["score", 100]})
      expect(expression.in_words).to eq("score is less than 100")
    end
    ...
    it "returns human-readable text for GreaterThanOrEqualTo expression" do
      expression = described_class.build({"GreaterThanOrEqualTo" => ["score", 100]})
    ...
    it "returns human-readable text for single condition Any expression" do
      expression = described_class.build({"Any" => [{"Equal" => ["single", "condition"]}]})
      expect(expression.in_words).to eq("single is equal to condition")
    end

    it "returns human-readable text for multiple condition Any expression" do
      expression = described_class.build({
        "Any" => [
          {"Equal" => ["plan", "basic"]},
    ...
    it "returns human-readable text for empty All expression" do
      expression = described_class.build({"All" => []})
      expect(expression.in_words).to eq("all 0 conditions")
    end
    ...
    it "returns human-readable text for multiple condition All expression" do
      expression = described_class.build({
        "All" => [
          {"Equal" => ["status", "active"]},
      ...
      expect(expression.in_words).to eq("all 2 conditions")
    end

    it "returns human-readable text for comparable Property expression" do
      expression = described_class.build({
        "Equal" => [
          {"Property" => ["age"]},
          21
